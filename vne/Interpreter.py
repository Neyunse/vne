...
class Interpreter:
    ...
    def __init__(self, lexer, renderer, config):
        ...
        self.command_table = {
            ...
            "dialogue": self.show_dialogue,  # Handles dialogue
        }
    ...
    def execute_next_command(self):
        ...
        if parsed_command["command"] in self.command_table:
            print(f"Executing: {parsed_command['command']}")  # Depuración
    ...
    def parse_command(self, command_line):
        ...
        if command_line.startswith('@'):
            ...
            if match:
                command = match.group(1)  # Command (e.g., @Load)
                arguments = match.group(2).strip()  # Arguments inside parentheses
    ...
    def show_dialogue(self, parsed_command):
        ...
        print(f"Dialogue: {dialogue_text}")  # Temporary for debugging
        # Here you would integrate with your renderer to show text on the screen
    ...
    def process_scene(self, parsed_command):
        ...
        print(f"Processing scene: {scene_key}")  # Depuración

        scene_path = self.lexer.assets["scenes"].get(scene_key)
        if not scene_path:
            raise ValueError(f"Scene '{scene_key}' not defined in scenes.kag")
        
        full_scene_path = os.path.normpath(os.path.join(self.config.base_game, "data", scene_path))
        print(f"Loading scene file: {full_scene_path}")  # Depuración
    ...
    def define_scene(self, parsed_command):
        ...
        scene_key = args[0].strip()  # Clave de la escena
        scene_file = args[1].strip().strip('"')  # Archivo de la escena
        
        scene_path = os.path.normpath(f"scenes/{scene_file}.kag")
        self.lexer.assets["scenes"][scene_key] = scene_path
        print(f"Scene defined: {scene_key} -> {scene_path}")  # Depuración
